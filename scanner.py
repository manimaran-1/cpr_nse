  ...
  def scan_market(symbols, interval='1d', progress_callback=None, close_method="Intraday Candle
  Close", target_session="Current Session"):
      """
  ...
  phase_label="intraday")
      ...
      if interval in ['1h', '15m', '5m', '1m', '30m']:
          ...
          daily_cache = data_loader.fetch_data_batch(symbols, interval='1d', max_workers=4,
                                                     progress_callback=_fetch_progress,
  phase_label="daily")
          logger.info(f"Phase 1b complete: {len(daily_cache)}/{total} daily in
  {_time.time()-t1b:.1f}s")
          ...
          if sample_df is not None:
              ...
              if len(unique_dates) >= 1:
                  if target_session == "Next Session":
                      ...
                      prev_trading_date = unique_dates[-2] if len(unique_dates) >= 2 else
  unique_dates[-1]

                  logger.info(f"Detected trading date for Bhavcopy lookup ({target_session}):
  {prev_trading_date}")
                  bhavcopy_lookup = data_loader.load_bhavcopy_lookup(prev_trading_date)
                  if not bhavcopy_lookup:
                      logger.warning(f"Bhavcopy lookup not resolved for date {prev_trading_date}.
  Falling back to default.")
              else:
                  logger.warning(f"Insufficient dates in dataset to determine baseline date:
  {unique_dates}")
          else:
      ...
      t_end = _time.time()
      logger.info(f"Scan complete: {len(results_df)} results from {len(data_cache) -
  skipped_count}/{len(data_cache)} fetched in {t_end-t0:.1f}s")
